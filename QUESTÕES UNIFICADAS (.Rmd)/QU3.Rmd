...
LI.var.perda <- ((n-1)*sd.perda^2)/qchisq(1-alpha/2, df = n-1, lower.tail = TRUE)
LS.var.perda <- ((n-1)*sd.perda^2)/qchisq(alpha/2, df = n-1, lower.tail = TRUE)
...
n <- ((qt(p = 0.05/2, df = 45, lower.tail = TRUE)*sd(LQQU3$Perda))/e)^2
print(round(n))
```
...
n <- ((qt(p = 0.05/2, df = 45, lower.tail = TRUE)*sd(LQQU3.1$Perda))/e)^2
print(round(n))
```
...
```{r}
t.test(x = LQQU3$Perda, alternative = "less", mu = 18, conf.level = 0.95)
t.test(x = LQQU3.1$Perda, alternative = "less", mu = 18, conf.level = 0.95)
```
...
```{r}
t.test(x = LQQU3$Perda, alternative = "less", mu = 18, conf.level = 0.99)
t.test(x = LQQU3.1$Perda, alternative = "less", mu = 18, conf.level = 0.99)
```

O aumento do nível de confiânça (diminuição do nível de significância alpha) torna inviável a rejeição da hipótese nula na qual a perda de vapor é igual a 18% para ambos os casos (com e sem outlier). Isso significa que, com base na teoria de teste de hipótese, é necessário alterar este procedimento, pois a média verdadeira da perda de vapor não será inferior a 18% em 99% das amostragens. Uma alternativa seria aumentar número de amostras buscando reduzir o intervalo de confiança.
...
![Algoritmo para construção de um modelo de regressão multivariado](QU3.ALGORITMO.JPG)