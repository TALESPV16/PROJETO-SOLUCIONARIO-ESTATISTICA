...
####Resposta letra b.2)
Considerando que os dados de IET seguem uma distribuição normal, temos:
```{r}
qnorm(p = 0.8, mean = mean(LQQU4.1$IET), sd = sd(LQQU4.1$IET))
```
...
```{r}
LQQU4.1 <- na.omit(LQQU4.1)

modelo <- lm(IET ~ Epoca + Clorofila + Celulas + Turbidez + Fosforo + pH + DBO5 + OD, data = LQQU4.1)
step <- stepAIC(object = modelo, direction = "both")
step$anova

modelo.QU4<- lm(IET ~ Clorofila + Turbidez + pH, data = LQQU4.1)
summary(modelo.QU4)
```
...
par(mfrow = c(2,2))
plot(x = LQQU4.1$IET, 
     y = modelo.QU4$residuals,
     xlab = "IET",
     ylab = "Resíduos",
     pch = 16,
     col = "red")
plot(x = LQQU4.1$Clorofila, 
     y = modelo.QU4$residuals,
     xlab = "Clorofila",
     ylab = "Resíduos",
     pch = 16,
     col = "green")
plot(x = LQQU4.1$Turbidez, 
     y = modelo.QU4$residuals,
     xlab = "Turbidez",
     ylab = "Resíduos",
     pch = 16,
...
par(mfrow = c(1,1))
hist(x = modelo.QU4$residuals,
     xlab = "Resíduos",
     ylab = "Densidade de Probabilidade",
     main = "",
     col = "lightgreen",
     probability = TRUE)
lines(density(modelo.QU4$residuals))

shapiro.test(modelo.QU4$residuals)
```

A validação do modelo demonstrou que as considerações necessárias para o método dos mínimos quadrados não foram atendidas. Uma análise descritiva dos dados objetivando a identificação e avaliação da remoção de outliers se faz necessária, a fim de garantir a aleatoriedade e nomalidade dos dados e dos resíduos do modelo proposto.  
